...
@Component({
  selector: 'app-top-comic-rcm',
  templateUrl: './top-comic-rcm.component.html',
  styleUrls: ['./top-comic-rcm.component.css']
})
export class TopComicRcmComponent implements OnInit{
  selectedFiles: File[] = [];
  comicDetail: Comic = new Comic();
  comicSearchRequest: Comic = new Comic();
  total: number = 0;
  asyncData: Comic[] | undefined;
  ...
  search() {
    this.comicService
      .searchMostView().subscribe(res => {
        console.log(res);
        if (FwError.THANHCONG == res.errorCode) {            
          if (res.data) {
            this.asyncData = res.data.map((comic) => {
              if (comic.image) {
                try {
